...
import { MENU as STATIC_MENU, type MenuItem, type MenuSection } from "@/data/menu";
import { Search, X, Flame, Sparkles, Plus, Minus, Trash2, QrCode, ArrowLeft } from "lucide-react";
...
import imgSoup from "@/assets/food/soup.jpg";
import imgRiceCurry from "@/assets/food/rice-curry.jpg";
import imgSunday from "@/assets/food/sunday.jpg";
import imgFriedRice from "@/assets/food/fried-rice.jpg";
import imgNoodles from "@/assets/food/noodles.jpg";
import imgKoththu from "@/assets/food/koththu.jpg";
import imgKoththuPlain from "@/assets/food/koththu-plain.jpg";
import imgDevilled from "@/assets/food/devilled.jpg";
import imgSeafood from "@/assets/food/seafood.jpg";
import imgSides from "@/assets/food/sides.jpg";
import imgDesserts from "@/assets/food/desserts.jpg";

const SECTION_IMG: Record<string, string> = {
  drinks: imgDrinks,
  soup: imgSoup,
  "rice-curry": imgRiceCurry,
  sunday: imgSunday,
  "fried-rice": imgFriedRice,
  noodles: imgNoodles,
  meegoreng: imgNoodles,
  nasi: imgFriedRice,
  "spicy-rice": imgFriedRice,
  koththu: imgKoththuPlain,
  "cheese-koththu": imgKoththu,
  prawns: imgDevilled,
  crab: imgSeafood,
  chicken: imgSunday,
  beef: imgDevilled,
...
function imageFor(id: string, title: string): string {
  ...
  if (/(koththu|kottu)/.test(s)) return imgKoththu;
  if (/(noodle|mee|pasta)/.test(s)) return imgNoodles;
  if (/(crab|lagoon)/.test(s)) return imgSeafood;
  if (/(prawn|devil|chilli|spicy|sisig)/.test(s)) return imgDevilled;
  if (/(fried rice|nasi|grill)/.test(s)) return imgFriedRice;
  if (/(rice|curry|dhal|stew)/.test(s)) return imgRiceCurry;
  if (/(soup)/.test(s)) return imgSoup;
  if (/(drink|juice|water|shake|milk|tea|coffee)/.test(s)) return imgDrinks;
  if (/(dessert|sweet|watalappan|pudding)/.test(s)) return imgDesserts;
  return imgSides;
}
...
export const Route = createFileRoute("/menu")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Full menu: rice & curry, koththu, devilled seafood, prawns, crab, chicken, beef. 113 Sri Lankan dishes from Trincomalee's coast." },
      { property: "og:title", content: "Menu — Geeth Me Restaurant" },
      { property: "og:description", content: "113 Sri Lankan dishes from Trincomalee's coast." },
      { property: "og:url", content: "https://geeth-me-restaurant-tco.lovable.app/menu" },
    ],
    links: [
      { rel: "canonical", href: "https://geeth-me-restaurant-tco.lovable.app/menu" },
    ],
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          inLanguage: "en",
          hasMenuSection: STATIC_MENU.map((s) => ({
            "@type": "MenuSection",
            name: s.title,
...
function Stepper({ qty, onDec, onInc, label }: { qty: number; onDec: () => void; onInc: () => void; label: string }) {
  return (
    <div className="inline-flex items-center gap-0.5 rounded-full bg-accent/15 border border-accent/40 p-0.5" aria-label={`${label}: ${qty}`}>
      <button aria-label={qty === 1 ? `Remove ${label}` : `Decrease ${label}`} onClick={onDec}
      ...
      <button aria-label={`Increase ${label}`} onClick={onInc}
  ...
  if (typeof p === "number") {
    ...
    return (
      <div className="flex items-center gap-2 shrink-0">
        <span className="rounded-full bg-accent/10 text-accent border border-accent/20 px-3 py-1 text-xs font-medium tabular-nums whitespace-nowrap">{fmt(p)}</span>
        {line ? (
          <Stepper qty={line.qty} label={item.name}
            onDec={() => setQty(item.n, undefined, line.qty - 1)}
            onInc={() => setQty(item.n, undefined, line.qty + 1)} />
        ) : (
          <button aria-label={`Add ${item.name}`} onClick={() => add(item)}
    ...
    <div className="flex flex-col items-end gap-1 shrink-0">
      {lineS ? (
        <div className="flex items-center gap-1.5">
          ...
          <Stepper qty={lineS.qty} label={`Small ${item.name}`} onDec={() => setQty(item.n, "s", lineS.qty - 1)} onInc={() => setQty(item.n, "s", lineS.qty + 1)} />
        </div>
      ) : (
        <button aria-label={`Add small ${item.name}`} onClick={() => add(item, "s")}
      ...
      {lineR ? (
        <div className="flex items-center gap-1.5">
          ...
          <Stepper qty={lineR.qty} label={`Regular ${item.name}`} onDec={() => setQty(item.n, "r", lineR.qty - 1)} onInc={() => setQty(item.n, "r", lineR.qty + 1)} />
        </div>
      ) : (
        <button aria-label={`Add regular ${item.name}`} onClick={() => add(item, "r")}
...
function MenuPage() {
  const { data: MENU = STATIC_MENU } = useMenu();
  ...
  useEffect(() => {
    ...
    MENU.forEach((s) => { const el = document.getElementById(s.id); if (el) obs.observe(el); });
  ...
  const sections: MenuSection[] = useMemo(() => {
    ...
    return MENU.map((s) => ({ ...s, items: s.items.filter((i) => i.name.toLowerCase().includes(needle) || s.title.toLowerCase().includes(needle)) })).filter((s) => s.items.length > 0);
  }, [q, MENU]);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="border-b border-border/60 bg-gradient-to-b from-card/40 to-background pt-20">
        <div className="mx-auto max-w-5xl px-4 pt-10 pb-10 md:pt-16 md:pb-14 relative overflow-hidden">
          <img src={imgDevilled} alt="" loading="lazy" className="hidden md:block absolute -top-4 right-4 w-36 h-36 rounded-2xl object-cover shadow-2xl rotate-[-6deg] icon-float ring-1 ring-border" />
          <img src={imgKoththu} alt="" loading="lazy" className="hidden md:block absolute top-28 right-44 w-28 h-28 rounded-2xl object-cover shadow-2xl rotate-[5deg] icon-float ring-1 ring-border" style={{ animationDelay: "0.6s" }} />
          <img src={imgSeafood} alt="" loading="lazy" className="hidden md:block absolute bottom-2 right-2 w-28 h-28 rounded-2xl object-cover shadow-2xl rotate-[-3deg] icon-float ring-1 ring-border" style={{ animationDelay: "1.2s" }} />
          ...
          <div className="relative max-w-2xl">
            ...
            <p className="mt-4 text-sm sm:text-base text-muted-foreground max-w-md">
              ...
              <span className="text-foreground">S</span> = small, <span className="text-foreground">R</span> = regular where shown.
            </p>
            ...
            <div className="mt-6 rounded-2xl border border-accent/25 bg-accent/5 px-4 py-3 flex items-start gap-3">
              ...
              <div className="text-xs sm:text-sm text-muted-foreground leading-relaxed">
                ...
                <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-accent text-accent-foreground"><Plus size={10} /></span>{" "}
      ...
      <div className="mx-auto max-w-5xl px-4">
        <div className="sticky top-20 md:top-24 z-30 -mx-4 px-4 pt-3 pb-2 bg-background/95 backdrop-blur border-b border-border/60">
          <div className="relative">
            ...
            {q && (
              <button aria-label="Clear" onClick={() => setQ("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"><X size={16} /></button>
            )}
          </div>
            ...
            {MENU.map((s) => {
              ...
              return (
                <a key={s.id} href={`#${s.id}`}
                  ref={(el) => { if (el && isActive) el.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" }); }}
                  ...
                  className={`shrink-0 inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-all duration-200 ${
                    isActive ? "border-accent bg-accent text-accent-foreground scale-105" : "border-border bg-card/60 text-muted-foreground hover:text-accent hover:border-accent/60"
                  }`}>
                  <img src={imageFor(s.id, s.title)} alt="" loading="lazy" className="w-5 h-5 rounded-full object-cover ring-1 ring-border" />
                  {s.title.replace(/ — .*/, "")}
                </a>
              );
            })}
        ...
        <div className="mt-8 space-y-12 pb-20">
          {sections.map((s) => {
            const banner = imageFor(s.id, s.title);
            return (
              <section key={s.id}>
                <div id={s.id} className="scroll-mt-44 rounded-2xl border border-border bg-card/40 overflow-hidden">
                  <header className="relative h-44 md:h-56 overflow-hidden border-b border-border/60">
                    <img src={banner} alt={s.title} loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
                    ...
                    <div className="relative h-full flex items-end justify-between gap-4 px-5 md:px-7 py-5">
                      <div className="min-w-0">
                        ...
                        <h2 className="font-display text-2xl md:text-3xl text-foreground leading-tight truncate drop-shadow-lg">{s.title}</h2>
                  ...
                  <ul className="grid md:grid-cols-2">
                    {s.items.map((i, idx) => (
                      <li key={i.n} className={`group relative px-5 md:px-7 py-4 flex items-start justify-between gap-4 hover:bg-card/70 transition-colors border-b border-border/40 md:[&:nth-last-child(-n+2)]:border-b-0 md:odd:border-r md:odd:border-border/40`}>
                        <div className="flex gap-3 min-w-0 flex-1">
                          <span className="text-[10px] text-muted-foreground/70 tabular-nums w-7 pt-1 shrink-0 font-medium tracking-wider">{String(i.n).padStart(2, "0")}</span>
                          <div className="min-w-0">
          ...
          {sections.length === 0 && (
            <div className="text-center py-16">
              <p className="text-muted-foreground">No dishes match "{q}".</p>
              <button onClick={() => setQ("")} className="mt-4 text-sm text-accent hover:underline">Clear search</button>
            </div>
          )}
        </div>
      </div>
